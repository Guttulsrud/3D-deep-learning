...
def conv_bn(x, filters, hp_name=None, hp=None):
    print(f'units_{hp_name}')
    if hp:
        filters = hp.Int(f'units_{hp_name}', min_value=filters * 2, max_value=filters * 4, step=32)

    x = layers.Conv1D(filters, kernel_size=1, padding="valid")(x)
    x = layers.BatchNormalization(momentum=0.0)(x)

    activation = hp.Choice(f'activation_{hp_name}', values=config['hpo']['activation']) if hp else 'relu'
